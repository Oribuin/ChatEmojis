...
public class MainMenu extends Menu {
    ...
    GuiScreen mainScreen() {
        ...
        FileConfiguration eConfig = emojiManager.getEmojiConfig();
        ConfigurationSection config = emojiManager.getEmojiSec();
        if (config == null)
            return null;

        // Create paginated section
        GuiScreenSection section = GuiFactory.createScreenSection(this.emojiSlots());
        ...
        Iterator<Integer> iterator = emojiSlots().iterator();

        for (String emoji : config.getKeys(false)) {
            ...
            guiScreen.setPaginatedSection(section.getSlotAmount(), this.emojiSlots().size() - 1, config.getKeys(false).size(), (pageNumber, startIndex, endIndex) -> {
                ...
                for (int i = startIndex; i <= endIndex; i++) {
                    ...
                    guiScreen.addButtonAt(iterator.next(), guiButton);
    ...
    private ItemStack getItem(String name) {
        ...
        itemMeta.setDisplayName(HexUtils.colorify(this.getValue(name + ".name")));

        List<String> lore = new ArrayList<>();
        for (String string : this.getMenuConfig().getStringList(name + ".lore"))
            lore.add(PlaceholderAPIHook.apply(player, HexUtils.colorify(string)));
    ...
    public String getValue(String configValue) {
        ...
        return PlaceholderAPIHook.apply(player, this.getMenuConfig().getString(configValue));
    }

    public String format(String msg, StringPlaceholders placeholders) {
        return placeholders.apply(PlaceholderAPIHook.apply(player, msg));
    }
